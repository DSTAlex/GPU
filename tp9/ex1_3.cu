...
int main(int argc, char const *argv[])
{
    ...
    kernel::compute<<<(N/4 + T - 1)/T,T, 0, s1>>>(dx,N/4);
    kernel::compute<<<(N/4 + T - 1)/T,T, 0, s2>>>(dx + (N/4),N/4);
    kernel::compute<<<(N/4 + T - 1)/T,T, 0, s3>>>(dx + 2*(N/4),N/4);
    kernel::compute<<<(N/4 + T - 1)/T,T, 0, s4>>>(dx + 3*(N/4),N/4);
