...
int main(int argc, char const *argv[])
{
    ...
    kernel3<<<(N/2 +T - 1)/T,T, 0 , s3>>>(dx,dy,dz,N/2);
    CUDA_CHECK( cudaGetLastError() );

    kernel3<<<(N/2 +T - 1)/T,T, 0 , s4>>>(dx + N/2, dy + N/2, dz + N/2,N/2);
    CUDA_CHECK( cudaGetLastError() );

    kernel4<<<(N/2 +T - 1)/T,T, 0, s3>>>(dz,N/2);
    CUDA_CHECK( cudaGetLastError() );

    kernel4<<<(N/2 +T - 1)/T,T, 0, s4>>>(dz + N/2,N/2);
    CUDA_CHECK( cudaGetLastError() );
